...
public class Playlist {
    ...
    public void addSongToPlaylist(String songName, String albumName) {
        for (Album album : albumArrayList) {
            if (album.getAlbumName().equals(albumName) && album.getSongArrayList().findSong(songName) != null) {
                playlistLinkedList.add(album.getSongArrayList().findSong(songName));
            }
...
//        }
        for (Album album : albumArrayList) {
            if (album.getSongArrayList().isSong(songName)) {
                playlistLinkedList.add(album.getSongArrayList().findSong(songName));
                isSongAdded = true;
            }
        }
